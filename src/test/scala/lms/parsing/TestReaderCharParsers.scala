...
trait CharReaderParsersProg extends CharReaderParsers with OptionOps with StringReaderOps {
...
class TestCharReaderParsers extends FileDiffSuite {
  ...
  def testSimpleParsers = {
    withOutFile(prefix + "char-reader-parser") {
      new CharReaderParsersProg with MyScalaOpsPkgExp with CharOpsExp
        with MyIfThenElseExpOpt with StructOpsFatExpOptCommon
        with ReaderParseResultOpsExp with FunctionsExp with OptionOpsExp
        with StringStructOpsExp with BarrierOpsExp with StringReaderOpsExp with MyScalaCompile { self =>

        val codegen = new MyScalaCodeGenPkg with ScalaGenCharOps with ScalaGenReaderParseResultOps
...
*/
  ...
  def testOr {
    withOutFile(prefix + "or-reader-parser") {
      new CharReaderParsersProg with MyScalaOpsPkgExp with CharOpsExp
        with MyIfThenElseExpOpt with StructOpsFatExpOptCommon
        with ReaderParseResultOpsExp with FunctionsExp with OptionOpsExp
        with StringStructOpsExp with BarrierOpsExp with StringReaderOpsExp with MyScalaCompile { self =>
    ...
    assertFileEqualsCheck(prefix + "or-reader-parser")
  }
}