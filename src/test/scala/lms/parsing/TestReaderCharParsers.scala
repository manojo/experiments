...
trait CharReaderParsersProg extends CharReaderParsers {
...
class TestCharReaderParsers extends FileDiffSuite {

  val prefix = "test-out/"

  def testSimpleParsers = {
    withOutFile(prefix + "char-reader-parser") {
      new CharReaderParsersProg with MyScalaOpsPkgExp with CharOpsExp with MyIfThenElseExpOpt with StructOpsFatExpOptCommon with ReaderParseResultOpsExp with FunctionsExp with OptionOpsExp with StringStructOpsExp with BarrierOpsExp with ReaderOpsExp with MyScalaCompile { self =>

        val codegen = new MyScalaCodeGenPkg with ScalaGenCharOps with ScalaGenReaderParseResultOps with ScalaGenFatStructOps with ScalaGenFunctions with ScalaGenOptionOps with ScalaGenStringStructOps with ScalaGenBarrierOps with ScalaGenIfThenElseFat with ScalaGenReaderOps {
      ...
      assertFileEqualsCheck(prefix + "char-reader-parser")
    }
  }
